...
class Database:
    ...
    def ensure_schema(self) -> None:
        ...
        try:
            with conn.cursor() as cursor:
                ...
                try:
                    cursor.execute(
                        "ALTER TABLE ipo ADD COLUMN announcement_sent TINYINT(1) DEFAULT 0"
                    )
                except mysql.connector.Error as exc:
                ...
                try:
                    cursor.execute(
                        "ALTER TABLE ipo ADD COLUMN allotment_url TEXT"
                    )
                except mysql.connector.Error as exc:
                    if exc.errno == errorcode.ER_DUP_FIELDNAME:
                        pass
    ...
    def get_unsent_announcements(self) -> List[Dict[str, str]]:
        query = "SELECT id, name, announcement_url FROM ipo WHERE announcement_sent = 0"
        conn = self._get_connection()
        try:
            with conn.cursor(dictionary=True) as cursor:
                cursor.execute(query)
...
def run_pipeline():
    ...
    try:
        ...
        return
    backfill_sent = send_pending_announcements(db, session)
    if backfill_sent:
        logger.info("Backfilled %s existing announcement urls", backfill_sent)
    ...
    logger.info(
        "Job finished, new records: %s, updated allotment_url: %s",
        new_count,
